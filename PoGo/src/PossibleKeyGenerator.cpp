...
PossibleKeyGenerator::PossibleKeyGenerator(KmerMap const* k)
: m_keys_generated(false), m_kmers(k) {}

int PossibleKeyGenerator::set_original_key(std::string const& key) {
	if (key.length() >= ((GENOME_MAPPER_GLOBALS::PEPTIDE_MAPPER::ALLOWED_MISMATCHES + 1) * GENOME_MAPPER_GLOBALS::PEPTIDE_MAPPER::KMER_LENGTH)) {
		...
		for (size_t i = 0; i <= GENOME_MAPPER_GLOBALS::PEPTIDE_MAPPER::ALLOWED_MISMATCHES; i++) {
			m_keys.push_back(key.substr((i * GENOME_MAPPER_GLOBALS::PEPTIDE_MAPPER::KMER_LENGTH), GENOME_MAPPER_GLOBALS::PEPTIDE_MAPPER::KMER_LENGTH));
		}
		m_keys_generated = true;
		m_curr_element = m_keys.begin();
		return 1;
	}
	if (key.length() >= GENOME_MAPPER_GLOBALS::PEPTIDE_MAPPER::KMER_LENGTH) {
		set_short_original_key(key.substr(0, GENOME_MAPPER_GLOBALS::PEPTIDE_MAPPER::KMER_LENGTH));
		return 0;
	}
	return -1;
}
...
void PossibleKeyGenerator::generate_keys_one_mismatch() {
	...
	for (size_t kmer_it = 0; kmer_it < GENOME_MAPPER_GLOBALS::PEPTIDE_MAPPER::KMER_LENGTH; kmer_it++) {
		char original = m_key[kmer_it];
		for (size_t aa_it = 0; aa_it < GENOME_MAPPER_GLOBALS::PEPTIDE_MAPPER::ALLOWED_AMINO_ACIDS.size(); aa_it++) {
			m_key[kmer_it] = GENOME_MAPPER_GLOBALS::PEPTIDE_MAPPER::ALLOWED_AMINO_ACIDS[aa_it];
			if (m_kmers->contains(m_key)) {
				m_keys.push_back(m_key);
			}
		}
		m_key[kmer_it] = original;
	}
}
...
void PossibleKeyGenerator::generate_keys_two_mismatches() {
	...
	for (size_t i = 0; i < GENOME_MAPPER_GLOBALS::PEPTIDE_MAPPER::KMER_LENGTH - 1; i++) {
		char original = m_key[i];
		for (size_t j = i; j < GENOME_MAPPER_GLOBALS::PEPTIDE_MAPPER::KMER_LENGTH; j++) {
